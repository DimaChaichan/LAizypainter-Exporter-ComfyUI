...
export class PromptVariable {
    ...
    self.variable = $el(
      ...
      [
        ...
        $el('option', {
          dataset: {
            id: "selectionImage"
          }, value: 'selectionImage', text: 'Selection Image'
        })
      ]
    )
    self.content = $el("div", {
  ...
  reset() {
    ...
    for (let i = 0; i < this.variable.children.length; i++) {
      const variableOption = this.variable.children[i];
      if (variableOption.dataset.id !== "none" &&
        variableOption.dataset.id !== "image" &&
        variableOption.dataset.id !== "selection" &&
        variableOption.dataset.id !== "selectionImage")
