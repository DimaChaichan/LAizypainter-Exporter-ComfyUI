        ...
        input.onchange = _ => {
            ...
            file.text().then(data => {
                ...
                if (lzyData.hasOwnProperty('config'))
                    self.configTab.setData(lzyData.config)

                self.variablesTab.reset();
                self.promptTab.reset();

                if (lzyData.hasOwnProperty('variables'))
                    self.variablesTab.setData(lzyData.variables)
            })
        };
        input.click();
    }
